...
public:
    int maxWalls(vector<int>& robots, vector<int>& distance, vector<int>& walls) {
        // couldn't figure it out for a long time and copy pasted the answer
        ...
        sort(robotDist.begin(), robotDist.end()); // sorts by position first
        ...
        for (int i = 0; i < n; i++) {
            ...
            // curPtr finds the first position >= robotPos (corresponding to lower_bound)
            ...
            if (i == 0) {
                ...
                int newsubLeft = max(subLeft + currentLeft, subRight - prevRight +
